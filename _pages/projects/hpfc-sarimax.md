...
Forward contracts are one of the most critical instruments in power trading — locking in future electricity prices provides revenue certainty and hedges against spot market volatility. An **Hourly Price Forward Curve (HPFC)** models these future prices at hourly granularity, often projected years ahead.

This project develops a full pipeline to construct an HPFC for the **German day-ahead electricity market (EPEX SPOT)**: from raw data exploration and stationarity testing, through SARIMAX model design and hyperparameter optimisation, to multi-strategy backtesting and a final 2024 hourly price forecast — scaled to an assumed Cal-27 forward market price.
...
The seasonal period `s=24` explicitly models the **daily cycle** in hourly data. Expanding to `s=168` (weekly) would capture richer seasonality but exceeds memory limits on a standard machine — a natural direction for future work on a compute cluster.
